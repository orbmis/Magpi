    ...
    public function getTables($injectionPoint, $numberColumns, $reflectedColumns)
    {
        ...
        if (count($tables) < $tablesCount) {
            ...
            while (!empty($tableName) && $i < 100) {
                $qualifier = ' FROM INFORMATION_SCHEMA.TABLES WHERE table_schema = 0x'.self::strToHex($dbName)
...
$inputFile = (PHP_SAPI === 'cli') ? $argv[1] : $_GET['inputfile'];
$outputFile = (PHP_SAPI === 'cli') ? $argv[2] : $_GET['logfile'];

$scan->startScan($inputFile, file($inputFile), $outputFile);
